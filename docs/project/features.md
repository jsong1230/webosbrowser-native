...
| F-03 | URL 입력 UI | Qt 입력 위젯으로 URL 입력 (리모컨 최적화) | Must | F-01 | PG-1 | URLBar 컴포넌트 | M1 | 🔄 진행중 |
| F-04 | 페이지 탐색 컨트롤 | 뒤로/앞으로/새로고침/홈 버튼 UI (Qt Widgets) | Must | F-02 | PG-1 | NavigationBar 컴포넌트 | M1 | 🔄 진행중 |
| F-05 | 로딩 인디케이터 | 페이지 로딩 중 프로그레스바 표시 (Qt ProgressBar) | Must | F-02 | PG-1 | LoadingBar 컴포넌트 | M1 | 🔄 진행중 |
